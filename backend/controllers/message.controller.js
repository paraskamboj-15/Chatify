...
import Message from "../models/message.model.js";
import { getReceiverSocketId, io } from "../socket/socket.js";
import { v2 as cloudinary } from "cloudinary"; // Added Import

// Ensure Cloudinary is configured (Add your cloud_name, api_key, etc. in .env)
cloudinary.config({
    ...
    try {
        ...
        if (req.file) {
            try {
                ...
                fileUrl = uploadResponse.secure_url;
                // Determine type based on cloudinary response or mimetype
                if (uploadResponse.resource_type === 'image') fileType = 'image';
                else if (uploadResponse.resource_type === 'video') fileType = 'video';
                else fileType = 'file';
            } catch (uploadError) {
